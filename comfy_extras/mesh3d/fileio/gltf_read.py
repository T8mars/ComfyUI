...
def _resolve_uri(uri: str, base_dir: str | None) -> bytes:
    ...
    path = os.path.normpath(os.path.join(base_dir, urllib.parse.unquote(uri)))
...
def _instance_matrices(gltf: dict, buffers: list[bytes], node: dict):
    ...
    count = min(a.shape[0] for a in (translation, rotation, scale) if a is not None)
    matrices = []
...
def _iter_mesh_nodes(gltf: dict, buffers: list[bytes], warn):
    ...
    while stack:
        ...
        if "mesh" in node:
            instances = _instance_matrices(gltf, buffers, node)
...
def load_scene_geometry(gltf: dict, buffers: list[bytes], warn) -> list[dict]:
    ...
    unsupported = required & _UNSUPPORTED_REQUIRED
    if unsupported:
        raise ValueError(
            f"glTF requires unsupported compressed-geometry extensions {sorted(unsupported)}; "
...
def load_gltf(data: bytes, base_dir: str | None, warn):
    ...
    if not version or (version.split(".")[0].isdigit() and int(version.split(".")[0]) < 2):
